...
const _updateProgress = (dt) => {
    ...
    const didFailLoading = Boolean(document.querySelector('#data-manager-error'))
    const didMountMainContent = isPageLoaded || didFailLoading

    loadingTime.value += isPageLoaded ?
        dt :
        dt / Math.max(4, percentage.value)

    loadingTimeAfterRendering.value += didMountMainContent ?
        dt :
        0
