...
const pillsControllerOptions = computed(() =>
    spyScrollSection.value?.category.sections.map(section => ({
        active: spyScrollSection.value.id === section.id,
        model: section
    })) || []
)
...
const tabControllerOptions = computed(() =>
    categories.value?.map(category => ({
        active: spyScrollSection.value?.category?.id === category.id,
        model: category
    })) || []
)
...
const onTabControllerLinkClicked = (category) => {
    const targetSection = navigateToCategory(category)
