<template>
    <div v-if="loadError"
         id="data-manager-error"
         class="data-manager-error">
        <h2>Content Loading Error</h2>
        <p v-html="loadError"/>
    </div>

    <slot v-else-if="settings"/>
</template>
...
onMounted(() => {
    _load()
        ...
        .catch((error) => {
            loadError.value = error.message || "An unknown data loading error occurred."
